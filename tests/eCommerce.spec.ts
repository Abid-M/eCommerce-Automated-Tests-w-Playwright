...
test.describe("eCommerce Tests", () => {
  ...
  for (const { code, discount } of couponData) {
    test(`Applying Coupon - '${code}'`, async ({ page, addItemLogoutFix, cartPOM }, testInfo) => {
      // Validates the entered coupon
      await (await cartPOM.EnterCoupon(code)).ApplyCoupon();
      ...
      const calculatedTotal = await cartPOM.CalculateTotal(code);
